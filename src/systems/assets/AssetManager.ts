...
export class AssetManager extends EventEmitter implements Disposable {
   ...
   */
  public loadTexture(url: string, priority: 'high' | 'normal' | 'low' = 'normal'): Promise<Texture> {
    // Check cache first
  ...
  public loadCubeTexture(
    ...
    const loadPromise = () => new Promise<Texture>((resolve, reject) => {
      this._cubeTextureLoader.load(
        ...
        undefined,
        error => {
          this._engine.logger.error(`Failed to load cube texture: ${name}`, error);
          reject(error);
        }
      );
    });
   ...
   */
  public loadModel(url: string, priority: 'high' | 'normal' | 'low' = 'normal'): Promise<GLTF> {
    // Check cache first
   ...
   */
  public loadAudio(url: string, priority: 'high' | 'normal' | 'low' = 'normal'): Promise<AudioBuffer> {
    // Check cache first
   ...
   */
  public loadJSON<T = unknown>(url: string, priority: 'high' | 'normal' | 'low' = 'normal'): Promise<T> {
    // Check cache first
  ...
  public releaseAsset(url: string, type: AssetType): void {
    ...
    switch (type) {
      case 'texture': cache = this._textureCache; break;
      case 'cubeTexture': cache = this._cubeTextureCache; break;
      case 'model': cache = this._modelCache; break;
      case 'audio': cache = this._audioCache; break;
      case 'json': cache = this._jsonCache; break;
      default: return;
    }
   ...
   */
  private _addToQueue<T>(loadFunc: () => Promise<T>, priority: 'high' | 'normal' | 'low'): void {
    const wrapped = async () => {
  ...
  public cloneModel(originalUrl: string): Group | null {
    ...
    if (!cached) {
      this._engine.logger.warn(`Cannot clone model, original not found: ${originalUrl}`);
      return null;
    }
