...
export class NetworkSystem extends EventEmitter implements System {
  ...
  public async init(): Promise<void> {
    ...
    // Create a new peer
    this._peer = new Peer({
      debug: this._engine.isDebugMode() ? 2 : 0,
    });
    ...
    this.isEnabled = true;
    this._engine.logger.info(
      `NetworkSystem initialized with peer ID: ${this._peerId}`
    );
  }
  ...
  private _handlePeerDisconnected = (): void => {
    this._engine.logger.info(
      "Peer disconnected from server, attempting to reconnect"
    );
    ...
    try {
      ...
      switch (message.type) {
        ...
        case MessageType.PLAYER_TRANSFORM:
          this._handlePlayerTransformMessage(
            peerId,
            message as PlayerTransformMessage
          );
          break;
   ...
   */
  private _handlePlayerTransformMessage(
    peerId: string,
    message: PlayerTransformMessage
  ): void {
   ...
   */
  private _handlePlayerJoinMessage(
    peerId: string,
    message: NetworkMessage
  ): void {
   ...
   */
  private _handlePlayerLeaveMessage(
    peerId: string,
    message: NetworkMessage
  ): void {
  ...
  public sendMessage(peerId: string, message: NetworkMessage): void {
    ...
    if (peerInfo && peerInfo.connection.open) {
      ...
      this._engine.logger.warn(
        `Cannot send message to peer ${peerId}: Not connected`
      );
    }
  }
  ...
  /**
   * Update system state - check for stale connections
   */
  public update(): void {
    if (!this.isEnabled) return;
